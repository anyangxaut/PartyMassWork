...
public class ScrollerUtils extends ViewGroup {
    ...
    public ScrollerUtils(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                int scrollToChildIndex = scrollY / mChildWidth;
                mVelocityTracker.computeCurrentVelocity(1000);
                float yVelocity = mVelocityTracker.getYVelocity();
                if (Math.abs(yVelocity) >= 50) {
                    mChildIndex = yVelocity > 0 ? mChildIndex - 1 : mChildIndex + 1;
                } else {
                    mChildIndex = (scrollY + mChildWidth / 2) / mChildWidth;
                }
                mChildIndex = Math.max(0, Math.min(mChildIndex, mChildrenSize - 1));
                int dy = mChildIndex * mChildWidth - scrollY;
                smoothScrollBy(dy, 0);
    ...
    private void smoothScrollBy(int dx, int dy) {
        mScroller.startScroll(getScrollY(), 0, dy, 0, 500);
        invalidate();
    }
